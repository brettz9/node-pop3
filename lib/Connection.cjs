...
class Pop3Connection extends EventEmitter {
  ...
  */
  ...
  _pushStream(buffer) {
    ...
    if (TERMINATOR_BUFFER.equals(buffer) || this._streamAtLineStart && TERMINATOR_BUFFER_ARRAY.slice(1).some(_buffer => _buffer.equals(buffer))) {
      this._endStream();
      return;
    }
  ...
  connect() {
    ...
    resolve, reject) => {
      ...
      this.on('error', err => {
        ...
        if (this._stream && typeof this._stream.destroy === 'function' && typeof this._stream.listenerCount === 'function' && this._stream.listenerCount('error') > 0) {
